...
def SSA(pop,M,c,d,dim,f):
    ...
    #     #各个个体Xi为圆心，指定半径Ri，划分各个个体Xi的社区区域，Xi在Oi随机选择个体来学习，生产新的个体Hi，然后根据适应度好坏进行选择 
        for i in range(pop):
            O=[]
            # -------Ri为0 
            Ri=np.linalg.norm((pX[sortIndex[0,i],:]-X[sortIndex[0,i],:])) #每个个体的社区的半径
            for j in range(pop):  #从Xi往前找
                if i!=j and np.linalg.norm((X[sortIndex[0,i],:] - X[sortIndex[0,j],:])) <= Ri:              #若两点之间距离小于等于Ri 则加入Oi中
                    O.append(sortIndex[0,j])
            ...
            if(len(O)!=0):  #如果社区内找到个体则进行社区学习
                ...
                #(11)
                H = X[sortIndex[0,i],:] + rand_r*(pX[sortIndex[0,rand_l],:] - X[sortIndex[0,O[rand_o]],:])
                #(12)
                if fit[sortIndex[0,i],0]>=fun(f,H):
                    X[sortIndex[0,i],:]=H
                    fit[sortIndex[0,i],0]=fun(f,H)
            #(13)更新
            if fit[sortIndex[0,i],0]<=pFit[sortIndex[0,i],0]:  #感觉有些问题 下标是否要用sortIndex[0,i]？------------
                pX[sortIndex[0,i],:]=X[sortIndex[0,i],:]
                pFit[sortIndex[0,i],:]=fit[sortIndex[0,i],:]
            if pFit[sortIndex[0,i],:]< fMin:
                fMin=pFit[sortIndex[0,i],:]
                bestX=pX[sortIndex[0,i],:]
            print(O)      
    ...
    #         O=[]
    #         # -------Ri为0 
    #         Ri=np.linalg.norm((pX[i,:]-X[i,:])) #每个个体的社区的半径
    #         for j in range(i+1,pop):  #从Xi往前找
    #             if np.linalg.norm((X[i,:] - X[j,:])) <= Ri:              #若两点之间距离小于等于Ri 则加入Oi中
    #                 O.append(j)
    #             else:
    #                 break
    ...
    #             rand_r=np.random.random()
    #             #(11)
    #             H = X[i,:] + rand_r*(pX[rand_l,:] - X[O[rand_o],:])
    #             #(12)
    #             if fit[i,0]>=fun(f,H):
    #                 X[i,:]=H
    #                 fit[i,0]=fun(f,H)
    #         #(13)更新
    #         if fit[i,0]<=pFit[i,0]:  #感觉有些问题 下标是否要用sortIndex[0,i]？------------
    #             pX[i,:]=X[i,:]
    #             pFit[i,:]=fit[i,:]
    #         if pFit[i,:]< fMin:
    #             fMin=pFit[i,:]
