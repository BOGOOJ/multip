...
def Bounds(s,Lb,Ub,bestX): #Lb是下限 ub是上限
    ...
    for i in range(len(s)):
        if temp[i]<Lb[0,i] or temp[i]>Ub[0,i]:
            ...
            print('----------------------------------------------------------------------------------------------')
            temp[i]=np.random.uniform(lowb,highb)
    return temp
    ...
    #2、根据可行性法则将种群分为可行解和不可行解后排序划分   ------------------------  没做    
        ...
        if r2 < 0.8: # 预警值较小，说明没有捕食者出现
            for i in range(pNum):
                ...
                fit[sortIndex[0,i],0] = F.f( X[sortIndex[0,i],:])   # 算新的适应度值   为什么要算两遍值-----------
            ...
            for i in range(pNum):
                ...
                fit[sortIndex[0,i],0] = F.f( X[sortIndex[0,i],:])  #为什么要算两遍值-----------
        ...
        for ii in range(pop-pNum):
            ...
            X[sortIndex[0,i],:] = Bounds(X[sortIndex[0,i],:],lb,ub,bestX)
            fit[sortIndex[0,i],0] = F.f( X[sortIndex[0,i],:])
        ...
        for j in range(len(b)):
            ...
            fit[sortIndex[0,b[j]],0] = F.f( X[sortIndex[0,b[j]]])
    ...
    # 有的O里是没有的  原本效率低
        ...
        for i in range(pop):
            ...
            if(len(O)!=0):  #如果社区内找到个体则进行社区学习
                ...
                #(12)
                if fit[XsortIndexI,0]>=F.f( H):
                    X[XsortIndexI,:]=H
                    fit[XsortIndexI,0]=F.f( H)
            #(13)更新
